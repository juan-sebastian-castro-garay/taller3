...
public class GeneradorTiquetes {
    ...
    public static Tiquete generarTiquete(Vuelo vuelo, Cliente cliente, int tarifa) {
        ...
        // Crea un nuevo tiquete con el código generado y los datos proporcionados
        Tiquete nuevoTiquete = new Tiquete(codigo, vuelo, cliente, tarifa);

        // Registra el código del tiquete para evitar duplicados en el futuro
        codigos.add(codigo);
    ...
    /**
     * Genera un código único de 7 dígitos que no haya sido utilizado antes.
     * @return Un código único de 7 dígitos.
     */
    private static String generarCodigoUnico() {
        int numero = (int) (Math.random() * 10000000); // Número aleatorio de hasta 7 dígitos
        String codigo = String.format("%07d", numero);  // Asegura que tenga exactamente 7 dígitos

        // Mientras el código ya exista, generar uno nuevo
    ...
    public static void registrarTiquete(Tiquete unTiquete) {
        // Registra el código del tiquete en el conjunto de códigos ya utilizados
     ...
     * @return Retorna true si ya se tenía registrado un tiquete con el código dado, false en caso contrario.
     */
    public static boolean validarTiquete(String codigoTiquete) {
        // Verifica si el código del tiquete ya existe en el conjunto de códigos
